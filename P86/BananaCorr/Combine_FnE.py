...
path_correct_errs = glob.glob('../../../X-shooter/P86_COSMOS_1/Data/*/Combined_OBs/NIR/*_avwCombined_*xOB_sig5_V1.fits')
									
path_correct_flux = glob.glob('../../../X-shooter/P86_COSMOS_1/Data/*/Combined_OBs/NIR_Banana/*_avwCombined_NIRBanana_*xOB_sig5_V1.fits')
																					# CP-1291751_avwCombined_NIRBanana_6xOB_sig5_V1.fits

print 'No of Error / Flux spectra: %s / %s' % (len(path_correct_errs), len(path_correct_flux))


for i in range(len(path_correct_errs)):
	obj = path_correct_errs[i].split('/')[6]
	...
	path_out = '../../../X-shooter/P86_COSMOS_1/Data/%s/Combined_OBs/NIR_corr' % obj
	if not os.path.exists(path_out):
		os.system('mkdir %s' % path_out)
	path_out_name = path_out+'/'+filename.replace('V1','V1_NIR_corr')
	print path_out_name
