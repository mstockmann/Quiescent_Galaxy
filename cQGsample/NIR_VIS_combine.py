...
Nspec = glob.glob('../../X-shooter/cQGsample/Objects/*')
...
for i in range(len(Nspec)):
	...
	### Rebinned ###
	# CDELT1 = 0.9
	path_NIR = glob.glob('../../X-shooter/cQGsample/Objects/%s/NIR_corr/*_V1_NIRcorr_wmrebin15_opt.fits' % (target))

	path_VIS = glob.glob('../../X-shooter/cQGsample/Objects/%s/VIS/*_V1_wmrebin45_opt.fits' % (target))


	# CDELT1 = 0.06
	# path_NIR = glob.glob('../../X-shooter/cQGsample/Objects/%s/NIR_corr/*_V1_NIR_corr_opt.fits' % (target))

	# path_VIS = glob.glob('../../X-shooter/cQGsample/Objects/%s/VIS/*_V1_wmrebin3_opt.fits' % (target))

	# print path_NIR
	# print path_VIS
	print 'target: %s, NIR / VIS = %s / %s' % (target,len(path_NIR),len(path_VIS))
	# raise
