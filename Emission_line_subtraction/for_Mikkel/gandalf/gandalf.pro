@~/idl/gandalf/sauron_colormap.pro

; Modified version by S. Zibetti (Oct.-Nov. 2013): 
; allow for fixed resolution in lambda (not only in velocity, as in the original version)
; see notes_on_gandalf_SZ.txt for a short description
...
FUNCTION BVLSN_Solve_pxf, A, b, degree, $
    ...
    ENDIF ELSE BEGIN               ; Fitting multiple templates
        ...
        ;BVLS, A, B, BND, soluz
        BVLS_EXTERNAL, A, B, BND, soluz
        ;;stop
        ;;soluz=IMSL_LINLSQ(B,A)
    ENDELSE
...
FUNCTION CREATE_TEMPLATES, EMISSION_SETUP=emission_setup, PARS=pars, NPIX=npix         ,$
    ...
    int_disp_pix=0
    if (int_disp gt 0) then int_disp_pix=int_disp/velscale
    ...
    ; [OIII]5007)
    for i = 0,nlines-1 do begin
        if (int_res gt 0) then int_disp_pix=int_res/emission_setup.lambda[i_lines[i]]*cspeed/velscale
    ...
    if i_slines[0] ne -1 then begin
        ...
        for i = 0,n_slines-1 do begin
            ...
            if (int_res gt 0) then int_disp_pix=int_res/l_sline*cspeed/velscale
...
PRO SET_CONSTRAINTS, GALAXY=galaxy, NOISE=noise, CSTAR=cstar,                                     $
        ...
        FOR_ERRORS=for_errors, INT_RES=int_res
    ...
    if not keyword_set(for_errors) then begin
        for i=0,nlines*2 -1, 2 do begin
            ;;parinfo[i].limits   = start_pars[i] + [-1d4,1d4]/velscale ; SZ Limits for Vgas (from km/s to pixels)
    ...
    if not keyword_set(log10) then begin
        if not keyword_set(reddening) then begin
            functargs={CSTAR:cstar, GALAXY:galaxy, NOISE:noise, EMISSION_SETUP:emission_setup, $
                ...
                INT_DISP:int_disp, INT_RES:int_res, $
                FOR_ERRORS:for_errors}
        endif else begin
            functargs={CSTAR:cstar, GALAXY:galaxy, NOISE:noise, EMISSION_SETUP:emission_setup,$
                KINSTARS:kinstars, VELSCALE:velscale, DEGREE:degree, MDEGREE:mdegree, $
                GOODPIXELS:goodpixels, L0_GAL:l0_gal, LSTEP_GAL:lstep_gal, $
                INT_DISP:int_disp, INT_RES:int_res, REDDENING:reddening, L0_TEMPL:l0_templ, $
                FOR_ERRORS:for_errors}
        endelse
    endif else begin
        if not keyword_set(reddening) then begin
            functargs={CSTAR:cstar, GALAXY:galaxy, NOISE:noise, EMISSION_SETUP:emission_setup, $
                KINSTARS:kinstars, VELSCALE:velscale, DEGREE:degree, MDEGREE:mdegree, $
                GOODPIXELS:goodpixels, L0_GAL:l0_gal, LSTEP_GAL:lstep_gal, $
                INT_DISP:int_disp, INT_RES:int_res, LOG10:log10, $
                FOR_ERRORS:for_errors}
        endif else begin
            functargs={CSTAR:cstar, GALAXY:galaxy, NOISE:noise, EMISSION_SETUP:emission_setup,$
                KINSTARS:kinstars, VELSCALE:velscale, DEGREE:degree, MDEGREE:mdegree, $
                GOODPIXELS:goodpixels, L0_GAL:l0_gal, LSTEP_GAL:lstep_gal, $
                INT_DISP:int_disp, INT_RES:int_res, LOG10:log10, REDDENING:reddening, L0_TEMPL:l0_templ, $
                FOR_ERRORS:for_errors}
        endelse
    endelse
...
FUNCTION FITFUNC_GAS, pars, CSTAR=cstar, GALAXY=galaxy, NOISE=noise, EMISSION_SETUP=emission_setup, $
    ...
    gaus = create_templates(EMISSION_SETUP=emission_setup,LSTEP_GAL=lstep_gal, NPIX=npix, PARS=eml_pars, $
        INT_DISP=int_disp, INT_RES=int_res, velscale=velscale, LOG10=log10, $
        FOR_ERRORS=for_errors)
...
PRO GANDALF, templates, galaxy, noise, velScale, sol, emission_setup, l0_gal, lstep_gal,       $
    ...
    IF n_elements(int_res) GT 0 AND int_disp gt 0 THEN $
        message, 'Cannot have constant spectral resolution and constant dispersion simultaneously: please define either int_disp or int_res'
    IF n_elements(int_res)  LE 0 THEN int_res = 0d ELSE int_res = int_res
    ...
    set_constraints, GALAXY=galaxy, NOISE=noise, CSTAR=cstar, KINSTARS=kinstars, $
        ...
        PARINFO=parinfo, FUNCTARGS=functargs, INT_DISP=int_disp, INT_RES=int_res, $
    ...
    resid = fitfunc_gas(best_pars,CSTAR=cstar, GALAXY=galaxy, NOISE=noise, $
        ...
        EMISSION_TEMPLATES=emission_templates, INT_DISP=int_disp, INT_RES=int_res, $
        LOG10=log10, REDDENING=reddening, L0_TEMPL=l0_templ)
    ;
    ...
    IF KEYWORD_SET(FOR_ERRORS) THEN BEGIN
        ...
        resid_2 = fitfunc_gas(best_pars_2,CSTAR=cstar, GALAXY=galaxy, NOISE=noise, $
            ...
            EMISSION_SETUP=emission_setup, L0_GAL=l0_gal, LSTEP_GAL=lstep_gal, $
            EMISSION_TEMPLATES=emission_templates_2, INT_DISP=int_disp, INT_RES=int_res, $
            LOG10=log10, REDDENING=reddening, L0_TEMPL=l0_templ, $
            FOR_ERRORS=for_errors)
        ;
        if total(noise) eq n_elements(galaxy) then begin
