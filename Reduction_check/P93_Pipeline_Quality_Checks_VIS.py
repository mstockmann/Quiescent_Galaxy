...
def QC_predict(period,target,reduct_type,No_OB):
	...
	path_predict = glob.glob('../../X-shooter/%s/Data/Reduction/%s/OB*/%s/Reduction/Output/xsh_predict_FMTCHK_RESID_TAB_LINES_*.fits' % (period,target,reduct_type))
...
def QC_orderpos(period,target,reduct_type,No_OB):
	path_orderpos = glob.glob('../../X-shooter/%s/Data/Reduction/%s/OB*/%s/Reduction/Output/xsh_orderpos_ORDERPOS_RESID_TAB_*.fits' % (period,target,reduct_type))
...
def QC_mflat(period,target,reduct_type,No_OB):
	path_mflat = glob.glob('../../X-shooter/%s/Data/Reduction/%s/OB*/%s/Reduction/Output/xsh_mflat_*_MFLAT_GRID_BACK_SLIT_*.fits' % (period,target,reduct_type))
	...
	# path_masterflat = glob.glob('../../X-shooter/%s/Data/Reduction/%s/OB*/%s/Reduction/Output/xsh_mflat_*_MASTER_FLAT_SLIT_*.fits' % (period,target,reduct_type))
	...
	for i in range(len(path_mflat)):	
		...
		# plt.subplot(gs[:1, :])
		# plt.imshow(Counts.T,vmin=-0.6,vmax=1.0,cmap='YlOrBr_r')
		# plt.axis([0,2000,0,1000])
		# plt.annotate('Example frame',xy=(100,900),color='white')
		# plt.title('xsh_mflat')
		# # plt.scatter(Y,X,s=0.2)
		...
		ext_name = '_'.join(path_mflat[i].split('/')[-1].split('_')[2:4])

		if not os.path.exists(path_out):
			os.system('mkdir %s' % path_out)

		plt.savefig(path_out+'/3-QC-mflat_%s.png' % ext_name )
	plt.close("all")
...
def QC_2dmap(period,target,reduct_type,No_OB):
	path_2dmap = glob.glob('../../X-shooter/%s/Data/Reduction/%s/OB*/%s/Reduction/Output/xsh_2dmap_WAVE_RESID_TAB_LINES_*.fits' % (period,target,reduct_type))

	print '2DMAP: %s. Number of exposures / 2dmap output: %s / %s' % (target,len(path_2dmap),No_OB)

	path_master_2dmap = glob.glob('../../X-shooter/%s/Data/Reduction/%s/OB*/%s/Reduction/Output/xsh_2dmap_WAVE_MAP_*.fits' % (period,target,reduct_type))
...
def QC_wavecal(period,target,reduct_type,No_OB):
	...
	path_wavecal = glob.glob('../../X-shooter/%s/Data/Reduction/%s/OB*/%s/Reduction/Output/xsh_wavecal_TILT_TAB_SLIT_*.fits' % (period,target,reduct_type))
...
def QC_response(period,target,reduct_type,No_OB):
	path_flux_cat = glob.glob('../../X-shooter/%s/Data/Reduction/%s/data_UVB_VIS/data_with_raw_calibs/M.XSHOOTER.2013-07-01T15:48:28.015.fits' % (period,target))

	path_response = glob.glob('../../X-shooter/%s/Data/Reduction/%s/OB*/%s/Reduction/Output/xsh_respon_slit_nod_FLUX_SLIT_FLUX_MERGE1D_*.fits' % (period,target,reduct_type))

	path_OBs = glob.glob('../../X-shooter/%s/Data/Reduction/%s/OB*' % (period,target))
	print 'RESPONSE: %s. Number of OBs / Response output: %s / %s' % (target,len(path_OBs),len(path_response))
...
# Note that the wavecal and flexcomp was not run for the vis spectra
...
path = glob.glob('../../X-shooter/%s/Data/Reduction/%s/OB*/%s/Reduction/Output/xsh_scired_slit_nod_SCI_SLIT_FLUX_MERGE2D_*.fits' % (period,target,reduct_type))
